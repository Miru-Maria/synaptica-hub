...
function LineContent({ line, type, words }: { line: string | null; type: string; words: WordSeg[] | null }) {
  const textClass =
    type === "removed" ? "text-red-200" :
    type === "added" ? "text-emerald-200" :
    "text-neutral-200";

  if (words) {
    return (
      <span className={`font-mono text-[13px] leading-5 break-all ${textClass}`}>
        {words.map((w, i) => (
          <span
            key={i}
            className={
              w.changed && type === "removed" ? "bg-red-500/40 rounded-[2px]" :
              w.changed && type === "added" ? "bg-emerald-500/40 rounded-[2px]" :
              ""
            }
          >
            {w.text}
        ...
        <div className="px-4 sm:px-6 py-3 flex items-center gap-4">
          ...
          {view === "compare" && stats && (
            <div className="flex items-center gap-3 ml-4 flex-wrap">
              <span className="text-xs px-2.5 py-1 bg-red-500/10 border border-red-500/20 text-red-400 rounded-full">
                −{stats.removed} removed
              </span>
              <span className="text-xs px-2.5 py-1 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-full">
                +{stats.added} added
              </span>
              <span className="text-xs px-2.5 py-1 bg-neutral-800 border border-neutral-700 text-neutral-400 rounded-full">
          ...
          {view === "compare" && (
            <button
              ...
              className="ml-auto flex items-center gap-1.5 text-sm text-neutral-400 hover:text-neutral-200 transition-colors"
            >
      ...
      {view === "input" ? (
        <main className="flex-1 px-4 sm:px-6 py-6 space-y-5 max-w-7xl mx-auto w-full">
          <p className="text-neutral-400 text-sm">
            Paste or upload two document versions to compare them side-by-side with word-level change highlighting.
          </p>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="flex shrink-0 border-b border-neutral-800">
            <div className="flex-1 flex items-center gap-2 px-4 py-2 border-r border-neutral-800 bg-red-500/5">
              <span className="text-xs font-semibold text-red-400 uppercase tracking-wider">Document A — Original</span>
            </div>
            <div className="flex-1 flex items-center gap-2 px-4 py-2 bg-emerald-500/5">
              <span className="text-xs font-semibold text-emerald-400 uppercase tracking-wider">Document B — Revised</span>
            </div>
          </div>
          ...
          <div className="flex-1 flex overflow-hidden" style={{ height: "calc(100vh - 105px)" }}>
            <div
              ...
              className="flex-1 overflow-y-auto overflow-x-auto border-r border-neutral-800"
            >
              {rows.map((row, i) => {
                const bg =
                  row.leftType === "removed" ? "bg-red-500/10" :
                  row.leftType === "empty" ? "bg-neutral-900/30" : "";
                const marker =
                  row.leftType === "removed" ? "−" :
                  row.leftType === "empty" ? " " : " ";
                const markerColor =
                  row.leftType === "removed" ? "text-red-500/60" : "text-transparent";

                return (
                  <div
                    key={i}
                    data-ridx={i}
                    className={`flex items-stretch min-h-[22px] ${bg} ${row.isChange && changeRowsRef.current[currentChange] === i ? "ring-1 ring-inset ring-neutral-500/40" : ""}`}
                  >
                    <span className="w-11 text-right text-[11px] text-neutral-600 pr-2.5 pt-0.5 shrink-0 select-none font-mono leading-5">
                      {row.leftNum ?? ""}
                    </span>
                    <span className={`w-4 text-[11px] shrink-0 select-none font-mono leading-5 pt-0.5 ${markerColor}`}>
                      {marker}
                    </span>
            ...
            >
              {rows.map((row, i) => {
                const bg =
                  row.rightType === "added" ? "bg-emerald-500/10" :
                  row.rightType === "empty" ? "bg-neutral-900/30" : "";
                const marker =
                  row.rightType === "added" ? "+" :
                  row.rightType === "empty" ? " " : " ";
                const markerColor =
                  row.rightType === "added" ? "text-emerald-500/60" : "text-transparent";
